...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message items:(NSArray *)items{
    if (self = [super init]) {
        ...
        self.titleFont = [UIFont systemFontOfSize:[UIFont systemFontSize]+2];
...
-(void)layoutMenuView{
    ...
    [UIView animateWithDuration:0.6 delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:0.75 options:0 animations:^{
...
-(void)layoutasGrid{
    CGFloat menuWidth = (self.view.bounds.size.width<self.view.bounds.size.height)?self.view.bounds.size.width*0.85:self.view.bounds.size.height*0.85;
    [self layoutTitleandMessagewithMenuWidth:menuWidth];
    ...
    [self.items enumerateObjectsUsingBlock:^(CTPopoutMenuItem * obj, NSUInteger idx, BOOL *stop) {
        ...
        [self.menuView addSubview:self.titleView];
        //[self.menuView addSubview:self.messageView];
        // CUSTOM UI TEXTFIELD TO REPLACE MESSAGE VIEW
        self.textField = [[UITextField alloc]init];
        self.textField.frame = self.messageView.frame;
        ...
        [self.menuView addSubview:self.textField];
    }];
}
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (self.selectedItemView != nil) {
        ...
        if ([self.delegate respondsToSelector:@selector(menu:willDismissWithSelectedItemAtIndex:andValue:)]) {
            [self.delegate menu:self willDismissWithSelectedItemAtIndex:index andValue:self.textField.text];
        }
        [self dismissMenu];
    }else{
        if (!CGRectContainsPoint(self.menuView.frame, point)) {
